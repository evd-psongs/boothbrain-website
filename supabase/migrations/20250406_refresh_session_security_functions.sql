-- Refresh session security helper functions to fix ambiguous references and return types
...
begin
  ...
  select
    ...
    sm.device_id,
    sm.status,
    coalesce(au.email, '')::text as participant_email,
    coalesce(p.full_name, ''::text) as participant_name
  from public.session_memberships sm
